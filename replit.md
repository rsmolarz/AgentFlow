...
- **Agents** (`/agents`) - Grid of AI agent cards with create/delete, search, provider badges (GPT/CLA/GEM), info banner explaining agents, "Chat" button on hover for each agent card. Enhanced create form with model options per provider, temperature/memory explanations, help text on every field. Advanced Configuration section: CrewAI-style role assignment (worker/researcher/writer/reviewer/coordinator/analyst), memory modules (conversation/summary/semantic/persistent with cross-thread and semantic search options), tool selection (8 tools: web search, code exec, file search, API requests, SQL query, calculator, knowledge base, image gen), guardrails (input validation, output moderation, PII detection, token limit), and agent handoffs toggle
- **Agent Chat/Playground** (`/agents/:id/chat`) - Full-screen chat interface for conversing with any agent. Features: message history with user/assistant bubbles, tool usage indicators (web search, code execution), suggested prompts on empty state, simulated streaming responses with token/duration tracking, copy/thumbs feedback buttons, Config sidebar showing model/provider/role/tools/memory, Clear conversation button. Supports markdown rendering in responses.
...
- **Workflow Editor** (`/workflows/:id/edit`) - Full-screen React Flow canvas with:
  - Drag-and-drop node palette with 16 node types: trigger, agent, condition, output, code, llm_call, transform, error_handler, delay, loop, human_review, merge, parallel, webhook, api_call, knowledge_query
