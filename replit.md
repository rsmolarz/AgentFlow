...
- **Bulk Execution**: Run a workflow on multiple CSV rows at once with progress tracking at `/bulk-execution`. DB table: `bulk_executions` (workflowId, status, totalRows, completedRows, failedRows, headers, results JSON array, duration). API: `POST /api/workflows/:id/bulk-run` (accepts rows + headers, max 500 rows, processes async with per-row progress updates), `GET /api/bulk-executions` (list), `GET /api/bulk-executions/:id` (detail with polling), `DELETE /api/bulk-executions/:id`. Frontend: CSV drag-and-drop upload with preview table, workflow selector, real-time progress bar (green/red segments), per-row expandable results with input/output/error, filter by status, export results as CSV, recent bulk runs table.
- **Output Formatter**: Post-processing workflow node that formats agent output as JSON, CSV, or Markdown. Node type `output_formatter` in workflow editor palette (fuchsia icon). Config options per format: JSON (structure: plain/array/envelope, indentation: 0/2/4 spaces, sort keys), CSV (delimiter: comma/semicolon/tab/pipe, include headers, quote all fields), Markdown (style: table/bullet list/headings/custom template with {{field}} placeholders). Live preview panel shows formatted sample data that updates in real-time as you change settings. Field selection via comma-separated list (leave empty for all). Zod enum updated in all 4 definition schemas.
